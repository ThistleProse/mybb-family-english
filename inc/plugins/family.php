...
<td class="trow2" style="padding: 10px; text-align: justify;">
<div style="width: 95%; margin: auto; padding: 8px;  font-size: 12px; line-height: 1.5em;" class="trow1">
 {$lang->family_add_familymember_desc}<br /><br />
		<form method="post" action="family.php" id="addfamily">
	<table cellspacing="3" cellpadding="3" class="tborder" style="width: 90%";>
		<tr>
			<td class="tcat" colspan="2">
		...
		while($fammember = $db->fetch_array($query)) {
			...
			if($famuser['uid'] == "0" OR empty($famuser)) {
				...
				if(empty($fammember['picture'])) {
					$fammember['img'] = "<div class=\"{$img_class}\"><a href=\"#description{$fammember['fmid']}\"><img src=\"{$mybb->settings['familycp_default_img']}\" class=\"family-picture\" /></a></div>";	
				}
				else {
					$fammember['img'] = "<div class=\"{$img_class}\"><a href=\"#description{$fammember['fmid']}\"><img src=\"{$fammember['picture']}\" class=\" family-picture\" /></a></div>";
				}
...
function family_index_start() {
	...
	if(!empty($mybb->user['uid'])) {
		...
		// set template
		$index_family = "";
		if(mysqli_num_rows($query) >= "1" && !empty($mybb->user["fid" . $field_username])) {
